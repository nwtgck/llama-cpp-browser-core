...
All profiles are single-threaded. `webgpu-wasm64-jspi` and `webgpu-wasm32-asyncify` are experimental and require device-specific testing. The latter uses 32-bit memory (4 GiB ceiling) and Asyncify, without requiring memory64 or JSPI. Use its example loader or `ccall(..., { async: true })` to await suspended native calls. Feature-detect WebGPU and Wasm support in the application; a profile does not certify any browser or device. A successful build, a synthetic-model smoke test, and support for a large production model are separate claims; consult the manifest for the checks actually performed.
